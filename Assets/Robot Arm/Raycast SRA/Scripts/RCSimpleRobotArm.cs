...
public class RCArticulation {
    ...
    ///  Constructor
    /// </summary>
    public RCArticulation(Transform articulation) {
...
public class RCSimpleRobotArm : Agent
{
    ...
    // Dictionary where the key is a string and the value is an RCArticulation
    public Dictionary<string, RCArticulation> articulations = new Dictionary<string, RCArticulation>();
    ...
    public int angle;
    public int n;
    private Vector3 negativeInfinity = new Vector3(-999, -999, -999);
    ...
    public void ResetTarget()
    {
        ...
        targetLocations = new List<Transform>();
        nearestTarget = null;
    }
    ...
    public void SetUpArticulations(){
        articulations.Add("armBase", new RCArticulation(armBase));
        articulations.Add("arm0", new RCArticulation(arm0));
        articulations.Add("arm1", new RCArticulation(arm1));
    }
    ...
    public void moveArticulation(string articulationName, float x, float y, float z){
        ...
        RCArticulation articulation = articulations[articulationName];
    ...
    /// <summary>
    /// Get the closest point to the origin from a list of points
    /// </summary>
    public Transform getClosestPoint(List<Transform> points, Vector3 origin){
        
        Transform closestPointTransform = null;
        Vector3 closestPoint = negativeInfinity;
    ...
    /// <summary>
    /// Check if the gripper is touching the target
    /// </summary>
    /// <returns>
    /// True if the gripper is touching the target, false otherwise
    /// </returns>
    bool isTouchingTarget(){
        ...
        Collider targetCollider = nearestTarget.GetComponent<Collider>();
        if (targetCollider == null)
        {
            // Raise an error if the target doesn't have a collider
            // Debug.LogError("The target doesn't have a collider");
        }
    ...
    /// <summary>
    /// Check if the gripper is touching the ground
    /// </summary>
    /// <returns>
    /// True if the gripper is touching the ground, false otherwise
    /// </returns>
    bool isTouchingGround(){
        ...
        foreach (Collider gripperCollider in gripperColliders)
        {
            if (gripperCollider.bounds.Intersects(groundCollider.bounds))
            {
                // Debug.Log("Gripper touched the target");
                return true;
            }
    ...
    /// <summary>
    /// Launch a raycast from origin in the given direction and return the point where the raycast hit the target
    /// <returns>
    /// Vector3 if the raycast hit the target, Vector3.negativeInfinity otherwise
    /// </returns>
    ...
    /// <summary>
    ///  Find the near targets with the agent's position as the origin
    /// </summary>
    public void findNearTargets(int n, float degrees)
    {
        // Give a reward for finding a target
        ...
        for (int i = 0; i < n; i++)
        {
            Vector3 directionPos = Quaternion.Euler(0, degrees * i, 0) * baseForward;
            Vector3 directionNeg = Quaternion.Euler(0, -degrees * i, 0) * baseForward;
            Transform raycastResult;

            // Middle raycast
            raycastResult = launchRaycast(transform.position, baseForward, 5, "target", Color.red);
            if (raycastResult != null && !targetLocations.Contains(raycastResult))
            {
                targetLocations.Add(raycastResult);
                AddReward(0.5f);
            }

            // Positive raycast
            raycastResult = launchRaycast(transform.position, directionPos, 5, "target", Color.green);
            if (raycastResult != null && !targetLocations.Contains(raycastResult))
            {
                targetLocations.Add(raycastResult);
                AddReward(0.5f);
            ...
            // Negative raycast
            raycastResult = launchRaycast(transform.position, directionNeg, 5, "target", Color.blue);
            if (raycastResult != null && !targetLocations.Contains(raycastResult))
            {
                targetLocations.Add(raycastResult);
                AddReward(0.5f);
    ...
    /// <summary>
    /// Nearest target within a list of target locations
    /// </summary>
    /// <returns>
    /// Nearest target if found, null otherwise
    /// </returns>
    public Transform getNearestTarget()
    {
        // Find the near targets (not necesarily all of them)
        findNearTargets(n, angle);

        // If no targets were found, return negative infinity
        ...
        Transform closestTarget = getClosestPoint(targetLocations, transform.position);
    ...
    public void DrawGripperLevelGuidelines()
    {
        ...
        Vector3 arm1ToTarget = Vector3.Normalize(nearestTarget.position - articulations["arm1"].rigidbody.worldCenterOfMass);
    ...
    public void DrawBaseLevelGuidelines(){
        ...
        Vector3 baseForward = Vector3.Normalize(armBase.transform.forward);
        baseForward.y = 0;
        Vector3 toTarget = Vector3.Normalize(nearestTarget.position - armBase.transform.position);
        toTarget.y = 0;
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // If a target has been found, add the position of the target to the observations
        if(nearestTarget != null){    
            // Position of the target (3)
            sensor.AddObservation(nearestTarget.localPosition);

            // Get the normalized direction from the arm base to the target (4)
            Vector3 baseForward = Vector3.Normalize(armBase.transform.forward);
            baseForward.y = 0;
            Vector3 toTarget = Vector3.Normalize(nearestTarget.position - armBase.transform.position);
            toTarget.y = 0;
            ...
            // Get the normalized direction from the arm1 to the target (4)
            Vector3 arm1Up = Vector3.Normalize(arm1.transform.up);
            Vector3 arm1ToTarget = Vector3.Normalize(nearestTarget.position - articulations["arm1"].rigidbody.worldCenterOfMass);
        ...
        // Rotation off each body part's rigidbody (4 * 3 = 12)
        foreach (string key in articulations.Keys)
        {
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        if (nearestTarget != null)
        {
            ...
            // Reward the agent for aligning the base with the target
            ...
            // Reward the agent for aligning the arm1 (gripper) with the target
            Vector3 arm1Up = Vector3.Normalize(arm1.transform.up);
            Vector3 arm1ToTarget = Vector3.Normalize(nearestTarget.position - articulations["arm1"].rigidbody.worldCenterOfMass);
    ...
    void FixedUpdate()
    {
        Transform newNearestTarget = getNearestTarget();
        
        if(newNearestTarget != null){
            nearestTarget = newNearestTarget;
        }

        // Check if there hasn't been a target found yet
        if(nearestTarget != null){
            // Debug.Log("Nearest target: " + nearestTarget.position);
            Debug.DrawLine(transform.position, nearestTarget.position, Color.yellow);
        }

        // Add a reward if the gripper is touching the target
        if(isTouchingTarget()){
            AddReward(1.0f);
            EndEpisode();
    ...
    //     // Set the nearest target to negative infinity
    //     nearestTarget = null;
